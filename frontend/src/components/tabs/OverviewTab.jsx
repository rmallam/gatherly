    ...
    return (
        <div>
            ...
            <div className="card" style={{ padding: '1.75rem', marginBottom: '2rem' }}>
                ...
                {(() => {
                    // Calculate costs from all tabs
                    const cateringCost = (event.catering?.items || []).reduce((sum, item) => sum + (parseFloat(item.cost) || 0), 0);
                    const decorationsCost = (event.decorations?.items || []).reduce((sum, item) => sum + (parseFloat(item.cost) || 0), 0);
                    const giftsCost = (event.gifts?.items || []).reduce((sum, item) => sum + (parseFloat(item.cost) || 0), 0);
                    const vendorsCost = (event.vendors?.items || []).reduce((sum, item) => sum + (parseFloat(item.cost) || 0), 0);

                    const totalBudget = cateringCost + decorationsCost + giftsCost + vendorsCost;

                    // For now, using simple paid tracking - can be enhanced later with payment records
                    const cateringPaid = (event.catering?.items || []).reduce((sum, item) => sum + (parseFloat(item.paid) || 0), 0);
                    const decorationsPaid = (event.decorations?.items || []).reduce((sum, item) => sum + (parseFloat(item.paid) || 0), 0);
                    const giftsPaid = (event.gifts?.items || []).reduce((sum, item) => sum + (parseFloat(item.paid) || 0), 0);
                    const vendorsPaid = (event.vendors?.items || []).reduce((sum, item) => sum + (parseFloat(item.paid) || 0), 0);

                    const totalPaid = cateringPaid + decorationsPaid + giftsPaid + vendorsPaid;
                    ...
                    return (
                        <>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
                                ...
                                <div style={{ background: 'var(--bg-secondary)', padding: '1rem', borderRadius: 'var(--radius-md)' }}>
                                    <div style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', marginBottom: '0.25rem' }}>Amount Paid</div>
                                ...
                                <div style={{ background: 'var(--bg-secondary)', padding: '1rem', borderRadius: 'var(--radius-md)' }}>
                                    ...
                                    <div style={{ fontSize: '1.5rem', fontWeight: 700, color: remaining > 0 ? 'var(--warning)' : 'var(--success)' }}>
                                        ${remaining.toFixed(2)}
                                    </div>
                                </div>
                            </div>
                            ...
                            {totalBudget > 0 && (
                                <div>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem', marginBottom: '0.5rem' }}>
                                        ...
                                        <span style={{ fontWeight: 600, color: 'var(--primary)' }}>{paidPercentage.toFixed(0)}% Paid</span>
                                    </div>
                                    <div style={{ height: '8px', background: 'var(--bg-secondary)', borderRadius: '999px', overflow: 'hidden' }}>
                                        <div style={{
                                            height: '100%',
                                            width: `${paidPercentage}%`,
                                            background: paidPercentage === 100 ? 'var(--success)' : 'linear-gradient(90deg, var(--primary) 0%, var(--primary-dark) 100%)',
                                    ...
                                    <div style={{ marginTop: '1.5rem', display: 'grid', gap: '0.5rem' }}>
                                        <div style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '0.25rem' }}>Budget Breakdown</div>
                                        {cateringCost > 0 && (
                                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem' }}>
                                                <span>🍽️ Catering</span>
                                                <span style={{ fontWeight: 600 }}>${cateringCost.toFixed(2)}</span>
                                            </div>
                                        )}
                                        {decorationsCost > 0 && (
                                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem' }}>
                                                <span>🎨 Decorations</span>
                                                <span style={{ fontWeight: 600 }}>${decorationsCost.toFixed(2)}</span>
                                            </div>
                                        )}
                                        {giftsCost > 0 && (
                                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem' }}>
                                                <span>🎁 Gifts</span>
                                                <span style={{ fontWeight: 600 }}>${giftsCost.toFixed(2)}</span>
                                            </div>
                                        )}
                                        {vendorsCost > 0 && (
                                            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem' }}>
                                                <span>🏢 Vendors</span>
                                                <span style={{ fontWeight: 600 }}>${vendorsCost.toFixed(2)}</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
                            ...
                            {totalBudget === 0 && (
                                <div style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-secondary)' }}>
                                    ...
                                    <div>No budget items added yet. Add costs in Catering, Decorations, Gifts, or Vendors tabs.</div>
                                </div>
                            )}
                        </>
                    );
